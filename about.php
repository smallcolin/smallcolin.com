<section id="aboutMe" class="about">
	<div class="container">
		<div class="row">
			<h3 class="animated flipInX">About Me</h3>
			<div class="col-xs-12 col-sm-6">
				<div class="about-text">
					<p>
						I am a 37 year old who is currently studying Web Development with CMS leanings at IT Högskola in Gothenburg, Sweden.
					</p>
					<p>
						So far I have experience with HTML, CSS, Javascript (Angular, jQuery, Polymer) and Wordpress based PHP. In my second year of school I will cover PHP/SQL in much greater depth and also courses on Drupal & Magento.
					</p>
					<p>
						I very much like to write code and learning new techniques & methods within programming languages.
						I also write and produce electronic music with computers.
					</p>
				</div>
			</div>
			<div class="col-xs-12 col-sm-6">
				<img src="assets/img/colin.jpg" alt="colin">
			</div>
		</div>
	</div>
</section>
